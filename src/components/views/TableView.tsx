    ...
    <Box>
      <ResponsiveBox breakpoint={"sm"}>
        ...
        <OfferTypeFilter
          ...
          setOfferTypeFilters={setOfferTypeFilters}
        />
      </ResponsiveBox>
      <Box marginY={2} />
      <TableContainer>
        <Table size={"small"}>
          ...
          <TableBody>
            {pagination.pageItems()?.map((item) => <OfferRow data={item} key={item.id} />) ??
              [...Array(pagination.itemsPerPage).keys()].map((it) => <OfferRowSkeleton key={it} />)}
          </TableBody>
        </Table>
      </TableContainer>
    </Box>
