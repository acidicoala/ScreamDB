...
export function Games() {
  ...
  useEffect(() => {
    ...
    sdk
      .searchGames({ keywords: keywords, sortBy: sortBy, sortDir: sortDir })
      ...
      .then((elements) => {
        setGames(
          elements.map<GameCardData>((element) => ({
            ...
            image: element.keyImages?.find((image) => image.type === KeyImageType.OfferImageTall)
              ?.url,
  ...
  return (
    <Box display={"flex"} flexDirection={"column"}>
      {" "}
      {games?.length === 0 ? (
        <SadFace children={locale.no_games} />
      ) : (
        <PaginatedContainer controls={pagination}>
          <ResponsiveBox breakpoint={"sm"}>
            ...
            >
              <Box display={"flex"} alignItems={"baseline"}>
                ...
                <Box marginX={1}>
                  <Typography children={locale.sort_by} />
                </Box>
          ...
          <Box className={classes.grid}>
            {pagination
              .pageItems()
              ?.map((it) => <GameCard data={it} style={{ margin: spacing(2) }} key={it.id} />) ??
